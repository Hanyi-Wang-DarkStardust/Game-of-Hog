...
def free_bacon(score):
    ...
    last_digit = (units_digit * tens_digit) % 10
    return 1 + last_digit
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for num_rolls in range(1,11):
        turn_score_func = make_averaged(roll_dice,num_samples)
        turn_score = turn_score_func(num_rolls,dice)
        result_list.append(turn_score)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    # BEGIN PROBLEM 11
    new_score = free_bacon(opponent_score)
    swaped = is_swap(score + new_score, opponent_score)
    if new_score >= margin:
        return 0
    if swaped and opponent_score > score + new_score:
        return 0
    return num_rolls
    # END PROBLEM 11
